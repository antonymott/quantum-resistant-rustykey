...
- **OPFS wallet:** browser keygen/sign **must** use `loadOpfsSkWallet()` (WebAuthn UV + PRF, ciphertext at rest, WASM in a Worker, COI required). Node `load*()` unchanged. [OPFS encrypted-sk wallet](../packages/opfs-sk).
