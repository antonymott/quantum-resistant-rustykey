...
sqisign_lvl5_public_key_bytes(void)
{
	return (int)CRYPTO_PUBLICKEYBYTES;
}

EMSCRIPTEN_KEEPALIVE int
sqisign_lvl5_private_key_bytes(void)
{
	return (int)CRYPTO_SECRETKEYBYTES;
}

EMSCRIPTEN_KEEPALIVE int
sqisign_lvl5_signature_bytes(void)
{
	return (int)CRYPTO_BYTES;
}

EMSCRIPTEN_KEEPALIVE int
sqisign_lvl5_seed_bytes(void)
{
	return 48;
}

EMSCRIPTEN_KEEPALIVE int
sqisign_lvl5_keypair_seeded(uint8_t *pk, uint8_t *sk, const uint8_t *seed)
{
...
EMSCRIPTEN_KEEPALIVE int
sqisign_lvl5_sign_seeded(uint8_t *sig, const uint8_t *msg, size_t msg_len,
...
EMSCRIPTEN_KEEPALIVE int
sqisign_lvl5_verify(const uint8_t *sig, size_t sig_len, const uint8_t *msg,
