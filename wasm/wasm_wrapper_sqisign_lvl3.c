...
sqisign_lvl3_public_key_bytes(void)
{
	return (int)CRYPTO_PUBLICKEYBYTES;
}

EMSCRIPTEN_KEEPALIVE int
sqisign_lvl3_private_key_bytes(void)
{
	return (int)CRYPTO_SECRETKEYBYTES;
}

EMSCRIPTEN_KEEPALIVE int
sqisign_lvl3_signature_bytes(void)
{
	return (int)CRYPTO_BYTES;
}

EMSCRIPTEN_KEEPALIVE int
sqisign_lvl3_seed_bytes(void)
{
	return 48;
}

EMSCRIPTEN_KEEPALIVE int
sqisign_lvl3_keypair_seeded(uint8_t *pk, uint8_t *sk, const uint8_t *seed)
{
...
EMSCRIPTEN_KEEPALIVE int
sqisign_lvl3_sign_seeded(uint8_t *sig, const uint8_t *msg, size_t msg_len,
...
EMSCRIPTEN_KEEPALIVE int
sqisign_lvl3_verify(const uint8_t *sig, size_t sig_len, const uint8_t *msg,
