/**
 * ML-KEM-512, a post-quantum key encapsulation mechanism in WebAssembly,
 ...
 */
import MLKEM512Module from "../wasm/build/wasm-module-512.js";

const ALGORITHM_NAME = "ML-KEM-512";
const JWK_ALG = "MLKEM512";

export type MlKemAlgorithm = { name: "ML-KEM-512" } | "ML-KEM-512";
...
async function exportKey(
  ...
  if (format === "spki") {
    ...
    return rawToDer(MLKEM512_SPKI, getPublicKeyDataRef(key)).buffer; // copy
  }
  ...
  if (format === "pkcs8") {
    ...
    // 2.3. Let result be the result of DER-encoding data.
    return rawToDer(MLKEM512_PKCS8, getPrivateKeyDataRef(key).privateSeedData)
      .buffer; // copy
  }
...
const MLKEM512_SPKI: DEREncoding = {
  fullLength: 822,
  prefix: new Uint8Array([
    0x30, 0x82, 0x03, 0x32, 0x30, 0x0b, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01,
    0x65, 0x03, 0x04, 0x04, 0x01, 0x03, 0x82, 0x03, 0x21, 0x00,
  ]),
};

const MLKEM512_PKCS8: DEREncoding = {
  fullLength: 86,
  prefix: new Uint8Array([
    0x30, 0x54, 0x02, 0x01, 0x00, 0x30, 0x0b, 0x06, 0x09, 0x60, 0x86, 0x48,
    0x01, 0x65, 0x03, 0x04, 0x04, 0x01, 0x04, 0x42, 0x80, 0x40,
  ]),
};
...
async function internalEncapsulate(
  ...
  // (Note: this is done by _mlkem512_enc_derand)
...
async function internalDecapsulate(
  ...
  // (Note: this is done by _mlkem512_dec)
  const secretKeyData =
